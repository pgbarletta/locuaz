...
def fixup_top(top_path: Path, name: str, *, target_chains: Iterable[str],
              binder_chains: Iterable[str],
              restraints: Dict[str, float],
              ) -> FileHandle:
    ...
    restraints_rest = restraints.get("rest", 1000)
...
"""
    ...
    [ position_restraints ]
    ;  i funct       fcx        fcy        fcz
    1    1       {restraints_rest}       {restraints_rest}       {restraints_rest}
"""
    ion_text = f"""     1     1  {restraints_rest}  {restraints_rest}  {restraints_rest}
"""
    mol_posres_files = {}
    ...
    for mol, mol_path in mol_itp_files.items():
        ...
        if mol in complex:
            file.write(header)
            restraint = restraints.get("target", 1000) if mol in target_chains else restraints.get("binder", 1000)
            with warnings.catch_warnings():
                warnings.simplefilter("ignore")
                u = mda.Universe(str(mol_path))
            at = u.select_atoms("not type H and not type H?")
            for a in at:
                file.write(f"{a.id:>6d}     1  {restraint}  {restraint}  {restraint}" "\n")
        elif mol in solvent:
