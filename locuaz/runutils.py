...
class MDrun:
    ...
    def min(cls, root_dir: Union[Path, DirHandle], *, gmx_mdrun: str, min_mdp: Path, gpu_id: int = 0,
            omp_threads: int = 2, mpi_threads: int = 1, pinoffset: int = 0, out_name="min",
            maxwarn: int = 0) -> "MDrun":

        obj = cls(DirHandle(root_dir), binary_path=gmx_mdrun, mdp=FileHandle(min_mdp), gpu_id=gpu_id,
                  num_threads_omp=omp_threads, num_threads_mpi=mpi_threads, pinoffset=pinoffset,
                  dev=f"-nb gpu -pin on -pinoffset {pinoffset}", out_name=out_name, maxwarn=maxwarn)
        return obj

    @classmethod
    def nvt(cls, root_dir: Union[Path, DirHandle], *, gmx_mdrun: str, nvt_mdp: Path, gpu_id: int = 0,
            omp_threads: int = 2, mpi_threads: int = 1, pinoffset: int = 0, out_name="nvt",
            maxwarn: int = 0) -> "MDrun":

        obj = cls(DirHandle(root_dir), binary_path=gmx_mdrun, mdp=FileHandle(nvt_mdp), gpu_id=gpu_id,
                  num_threads_omp=omp_threads, num_threads_mpi=mpi_threads, pinoffset=pinoffset,
    ...
    def __call__(self, complex: GROComplex) -> Tuple[bool, GROComplex]:
        ...
        try:
            p = sp.run(comando_md, stdout=sp.PIPE, stderr=sp.PIPE, cwd=Path(self.dir), shell=True, text=True)
        ...
        new_complex = type(complex).from_gro_zip(name=self.out_name, input_dir=Path(self.dir),
                                                 target_chains=complex.top.target_chains,
                                                 binder_chains=complex.top.binder_chains, gmx_bin="gmx")
...
def auto_md_params(config_md: Dict[str, any], all_threads: List[int],
                   nbranches: int) -> Tuple[List[int], int, int, int, int]:
    numa_regions = config_md["numa_regions"]
    ngpus = get_ngpus()
    numa_threads, numa_ngpus, numa_nbranches = numa_partition(all_threads, ngpus, nbranches, numa_regions)
    ...
    for nbranches_parallel in range(numa_nbranches, 0, -1):
        try:
            ...
            max_threads_per_branch = (len(numa_threads) - nbranches_parallel) // nbranches_parallel
        except ZeroDivisionError:
            raise RuntimeError("Need at least 2 threads to use as OMP_THREADS. This shouldn't happen.")
    ...
    numa_pin_offsets = np.array(numa_threads[0:-1:threads_per_branch][0:nbranches_per_gpu])
    # And now all of them:
    numa_step = len(all_threads) // numa_regions
    pin_offsets = list(chain.from_iterable([list(numa_pin_offsets + i * numa_step) for i in range(numa_regions)]))
...
def get_md_params(config_md: Dict[str, any], epoch: Epoch) -> Tuple[Dict[str, BranchMDParams], int]:
    nbranches = len(epoch)
    epoch_md_params = {}
    if config_md["mps"]:
        all_threads = sorted(list(os.sched_getaffinity(0)))
        pin_offsets, ngpus, omp_threads, mpi_threads, max_parallel_workers = auto_md_params(config_md,
                                                                                            all_threads, nbranches)
        for idx, (branch_name, branch) in enumerate(epoch.items()):
            epoch_md_params[branch_name] = BranchMDParams(gpu_id=idx % ngpus,
        ...
        for idx, (branch_name, branch) in enumerate(epoch.items()):
            epoch_md_params[branch_name] = BranchMDParams(gpu_id=idx % config_md["ngpus"],
